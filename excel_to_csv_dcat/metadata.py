...
def generate_dcat_metadata(
    ...
    if existing_dataset_uri and validate_uri(existing_dataset_uri):
        ...
        if existing_dataset_uri:  # It was provided but invalid
            logger.warning(f"Provided existing_dataset_uri '{existing_dataset_uri}' is invalid. Creating a new dataset description.")
        ...
        if os.path.exists(csv_file_path) and column_headers:
            try:
                ...
                for header_name_from_df in df_sample.columns:
                    ...
                    if original_header_name in validated_column_types:                        # Convert XSD string to URIRef
                ...
                logger.warning(f"CSV file '{csv_file_path}' is empty. Cannot infer column types.")
                for header_name in column_headers:
                    inferred_column_types[str(header_name)] = XSD.string
            except FileNotFoundError:
                logger.warning(f"CSV file '{csv_file_path}' not found. Cannot infer column types.")
                for header_name in column_headers:
                    inferred_column_types[str(header_name)] = XSD.string
            except Exception as e:
                logger.warning(f"Error reading or inferring types for '{csv_file_path}': {e}. Defaulting types to string.")
                for header_name in column_headers:
                    inferred_column_types[str(header_name)] = XSD.string
        else:
            for header_name in column_headers:
